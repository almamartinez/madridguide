...
import io.keepcoding.madridguide.manager.net.ActivityResponse;
import io.keepcoding.madridguide.model.Activity;
import io.keepcoding.madridguide.model.mappers.ActivityEntityActivityMapper;

public class ActivityEntityActivityMapperTests extends AndroidTestCase {
    ...
    public void testActivityEntityMappedIntoActivitiesWorksWithNullData() {
        ActivityEntityActivityMapper mapper = new ActivityEntityActivityMapper();

        List<Activity> activities = mapper.map(null);
    ...
    public void testShopEntityMappedIntoShopsWorksWithData() {
        ...
        List<Activity> activities = mapper.map(parseResponse(TEST_STRING));

        assertNotNull(activities);
        assertEquals(4, activities.size());
        assertEquals(310, activities.get(0).getId());
        assertEquals(221, activities.get(3).getId());
        assertEquals(40.4156535f, activities.get(3).getLatitude());
    ...
    private List<ActivityEntity> parseResponse(String response) {
        ...
        try {
            ...
            ActivityResponse shopResponse = gson.fromJson(reader, ActivityResponse.class);
